	...
	press100As() {
		for (let i = 0; i <= 100; i++) {
			...
			if (dividesX.dividesA && dividesY.dividesA) {
				...
				if (totalAPresses <= 100 && matchingDivisibility) {
			...
			if (bDividesX.dividesB && bDividesY.dividesB) {
				...
				console.log(`Found a solution!`);
				if (totalBPresses <= 100 && sameDivisibility) {
					this.solutions.push({ numA: this.m.numPressedA, numB: totalBPresses });
				} else console.log(`Too many b presses to get there (${totalBPresses} b presses)`);
			}
